...
export const ProductService = {
    // Buscar todos os produtos (cached)
    async getAll(): Promise<Product[]> {
        return dataCache.getOrFetch('products:all', async () => {
...
export const CustomerService = {
    // Buscar todos os clientes (cached)
    async getAll(): Promise<Customer[]> {
        return dataCache.getOrFetch('customers:all', async () => {
            const { data, error } = await supabase
                .from('customers')
                .select('*')
                .eq('is_active', true)
...
export const UserService = {
    // Buscar todos os usuários (cached)
    async getAll(): Promise<User[]> {
        return dataCache.getOrFetch('users:all', async () => {
            const { data, error } = await supabase
                .from('users')
                .select('*')
                .order('name', { ascending: true });
...
export const TransactionService = {
    ...
    // Buscar todas as transações COM itens (cached)
    async getAll(): Promise<Transaction[]> {
        return dataCache.getOrFetch('transactions:all', async () => {
            const { data, error } = await supabase
                .from('transactions')
...
export const DeliveryService = {
    async getAll(): Promise<DeliveryOrder[]> {
        return dataCache.getOrFetch('deliveries:all', async () => {
