...
const COLORS = ['rgb(var(--primary) / 0.3)','#ec4899','#8b5cf6','#14b8a6','#f59e0b','#ef4444'];
...
const Reports: React.FC = () => {
 ...
 // KPI Calculations
 // IMPORTANTE: usar t.total (que já inclui taxa de entrega) para coincidir com o financeiro do caixa
 ...
 <p className="text-slate-500 dark:text-slate-400">Análise detalhada de performance.</p>
 </div>
 <DSButton
 ...
 <div className="flex flex-col md:flex-row gap-4 justify-between">
  ...
  <SmoothTabs
  tabs={[
  { key: 'general', label: 'Geral / Loja', icon: <Store size={16} /> },
  { key: 'ecommerce', label: 'E-commerce', icon: <Globe size={16} /> },
  ]}
  activeKey={reportType}
  onChange={(k) => setReportType(k as 'general' | 'ecommerce')}
  />
 ...
 <p className="text-sm text-slate-500 dark:text-slate-400 mb-1">Faturamento Bruto</p>
 <h3 className="text-2xl font-bold text-slate-800 dark:text-white">R$ {totalRevenue.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</h3>
 <div className="mt-2 text-xs text-slate-400 flex items-center gap-1 bg-slate-50 dark:bg-slate-900/30 w-fit px-2 py-0.5 rounded">
 -- vs anterior
 </div>
 </div>
 <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm transition-colors">
 <p className="text-sm text-slate-500 dark:text-slate-400 mb-1">Total Pedidos</p>
 <h3 className="text-2xl font-bold text-slate-800 dark:text-white">{totalOrders}</h3>
 </div>
 <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm transition-colors">
 <p className="text-sm text-slate-500 dark:text-slate-400 mb-1">Ticket Médio</p>
 <h3 className="text-2xl font-bold text-slate-800 dark:text-white">R$ {averageTicket.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</h3>
 </div>
 <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm transition-colors">
 <p className="text-sm text-slate-500 dark:text-slate-400 mb-1">Categoria Top</p>
 <h3 className="text-2xl font-bold text-primary dark:text-primary">{topCategory}</h3>
 </div>
 ...
 <h3 className="font-bold text-slate-800 dark:text-white">Evolução de Vendas (Geral)</h3>
 <span className="text-xs text-slate-400 bg-slate-50 dark:bg-slate-700/50 px-2 py-1 rounded">{getPeriodLabel()}</span>
 </div>
 ...
 <stop offset="5%"stopColor="rgb(var(--primary) / 0.3)"stopOpacity={0.8} />
 <stop offset="95%"stopColor="rgb(var(--primary) / 0.3)"stopOpacity={0} />
 </linearGradient>
 </defs>
 ...
 <h3 className="font-bold text-slate-800 dark:text-white mb-4">Formas de Pagamento</h3>
 ...
 <span className="font-bold text-slate-800 dark:text-white">R$ {item.value.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>
 <span className="ml-1 text-xs text-slate-400">({totalPay > 0 ? ((item.value / totalPay) * 100).toFixed(0) : 0}%)</span>
 </div>
 </div>
 ));
 })()}
 ...
 <div>
 <p className="text-sm text-slate-500 dark:text-slate-400 mb-1">Vendas Online</p>
 ...
 <div className="flex justify-between items-start">
 <div>
 <p className="text-sm text-slate-500 dark:text-slate-400 mb-1">Taxa Conversão</p>
 ...
 <div className="flex justify-between items-start">
 <div>
 <p className="text-sm text-slate-500 dark:text-slate-400 mb-1">Ticket Médio (Site)</p>
 <h3 className="text-2xl font-bold text-slate-800 dark:text-white">R$ 0,00</h3>
 </div>
 <div className="text-xs text-slate-400 font-bold bg-slate-100 dark:bg-slate-700 px-2 py-1 rounded">--</div>
 </div>
 </div>
 <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm transition-colors">
 <div className="flex justify-between items-start">
 <div>
 <p className="text-sm text-slate-500 dark:text-slate-400 mb-1">Carrinhos Aband.</p>
 <h3 className="text-2xl font-bold text-slate-800 dark:text-white">0</h3>
 </div>
 ...
 <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-md relative animate-in fade-in zoom-in-95 duration-200">
 ...
 <h3 className="font-bold text-lg text-slate-800 dark:text-white">Exportar Relatório ({reportType ==='general'?'Geral':'E-commerce'})</h3>
 ...
 <div>
 <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Período Selecionado</label>
 <div className="p-3 bg-slate-100 dark:bg-slate-700 rounded-lg text-slate-600 dark:text-slate-300 text-sm flex items-center gap-2">
 <Calendar size={16} /> {getPeriodLabel()}
 </div>
 </div>

 <div>
 <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-3">Dados para Incluir</label>
 ...
 <input
 type="checkbox"
 className="rounded text-primary focus:ring-primary mr-3 h-4 w-4"
 ...
 <div>
 <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-3">Formato do Arquivo</label>
 <div className="grid grid-cols-2 gap-3">
 <div
 onClick={() => setDownloadConfig({ ...downloadConfig, format:'pdf'})}
 className={`cursor-pointer border rounded-lg p-3 flex flex-col items-center justify-center transition-all ${downloadConfig.format ==='pdf'?'border-primary bg-primary-50 dark:bg-primary/900/30 text-primary-700 dark:text-primary ring-1 ring-primary500':'border-slate-200 dark:border-slate-600 hover:border-slate-300 dark:hover:border-slate-500 text-slate-500 dark:text-slate-400'}`}
 >
 ...
 className={`cursor-pointer border rounded-lg p-3 flex flex-col items-center justify-center transition-all ${downloadConfig.format ==='excel'?'border-emerald-500 bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 ring-1 ring-emerald-500':'border-slate-200 dark:border-slate-600 hover:border-slate-300 dark:hover:border-slate-500 text-slate-500 dark:text-slate-400'}`}
 >
 ...
 <div className="p-4 border-t border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-700/30 flex gap-3">
 <button
