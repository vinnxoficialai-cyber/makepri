...
const POS: React.FC<POSProps> = ({ onAddDelivery, user }) => {
    ...
    const addItemToCart = (product: Product, variation?: ProductVariation) => {
        ...
        if (currentStock <= 0) {
            if (!window.confirm(`⚠️ "${product.name}${variation ? ` (${variation.name})` : ''}" está sem estoque (${currentStock} un).\n\nDeseja adicionar mesmo assim?`)) {
                return;
            }
        } else if (cartQty >= currentStock) {
            if (!window.confirm(`⚠️ "${product.name}${variation ? ` (${variation.name})` : ''}" tem apenas ${currentStock} un em estoque e você já adicionou ${cartQty}.\n\nDeseja adicionar mais?`)) {
                return;
            }
        }
