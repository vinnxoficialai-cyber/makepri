...
import { SectionHeader, EmptyState } from'../components/ds/layout';
...
const Inventory: React.FC<InventoryProps> = ({ user, autoFilterStalled, resetAutoFilter, autoFilterLowStock, resetAutoFilterLowStock }) => {
 ...
 if (totalRates >= 100) {
 toast.info("A soma das taxas e margem não pode ser 100% ou mais.");
 return;
 }
 ...
 if (!newProduct.priceSale || Number(newProduct.priceSale) <= 0) {
 ...
 toast.warning("Nome da variação é obrigatório (ex: P, M, Vermelho)");
 return;
 }
 ...
 <div>
 <h2 className="text-2xl font-bold text-slate-800 dark:text-white">Controle de Estoque</h2>
 <p className="text-slate-500 dark:text-slate-400">Gerencie produtos, fornecedores e valores.</p>
 </div>
 <div className="flex gap-3">
 <DSButton onClick={() => setIsAnalysisModalOpen(true)} variant="primary" startIcon={<Sparkles size={18} />}>Inteligência</DSButton>
 <DSButton onClick={() => { setNewProduct({ category: ProductCategory.CLOTHING, unit:"un", stock: 0, minStock: 10, commissionRate: 0, imageUrl:""}); setIsModalOpen(true); }} variant="primary" startIcon={<Plus size={18} />}>Novo Produto</DSButton>
 </div>
 </div>
 ...
 <p className="text-sm text-slate-500 dark:text-slate-400 mb-1">Valor Total em Estoque (Venda)</p>
 <h3 className="text-2xl font-bold text-slate-800 dark:text-white">R$ {totalValueSale.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</h3>
 ...
 <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm flex flex-col justify-between transition-colors">
 <div>
 <p className="text-xs text-slate-500 dark:text-slate-400 mb-1">Total de Itens</p>
 <h3 className="text-xl font-bold text-slate-800 dark:text-white">{totalItems}</h3>
 </div>
 ...
 <div className={`${isSalesperson ?'lg:col-span-3':'lg:col-span-2'} bg-white dark:bg-slate-800 p-5 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm transition-colors`}>
 <SectionHeader title="Distribuição de Estoque" />
 ...
 />
 <Bar dataKey="stock"fill="rgb(var(--primary) / 0.3)"radius={[0, 4, 4, 0]} barSize={20} />
 </BarChart>
 </ResponsiveContainer>
 </div>
 </div>
 ...
 <div className="bg-transparent lg:bg-white lg:dark:bg-slate-800 rounded-xl lg:shadow-sm lg:border border-slate-100 dark:border-slate-700 overflow-hidden flex flex-col transition-colors">
 {/* Toolbar */}
 <div className="p-4 border-b border-slate-100 dark:border-slate-700 flex flex-col lg:flex-row gap-4 justify-between items-center bg-slate-50/30 dark:bg-slate-700/30 lg:bg-slate-50/30 rounded-xl lg:rounded-none mb-4 lg:mb-0">
 <div className="relative w-full lg:w-96">
 <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-slate-400"size={18} />
 <input
 type="text"
 placeholder="Buscar por código, nome..."
 className="w-full pl-10 pr-4 py-2 border border-slate-200 dark:border-slate-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-[rgb(var(--primary) / 0.3)]/50 focus:border-primary/30 text-sm bg-white dark:bg-slate-700 text-slate-900 dark:text-white"
 value={searchTerm}
 ...
 />
  ...
  tabs={[
  { key: 'all', label: 'Todos' },
  { key: 'stalled', label: 'Itens Parados', icon: <AlertTriangle size={14} /> },
  { key: 'lowstock', label: 'Estoque Crítico', icon: <AlertTriangle size={14} /> },
  { key: 'promo', label: 'Em Promoção', icon: <Percent size={14} /> },
  ]}
  activeKey={showStalledOnly ? 'stalled' : showLowStockOnly ? 'lowstock' : showPromotionOnly ? 'promo' : 'all'}
  onChange={(k) => {
  setShowStalledOnly(k === 'stalled');
  setShowLowStockOnly(k === 'lowstock');
  setShowPromotionOnly(k === 'promo');
  }}
  size="sm"
  />
  <CustomDropdown
 ...
 <div
 key={product.id}
 className={`bg-white dark:bg-slate-800 p-4 rounded-2xl shadow-sm border flex items-center gap-4 relative overflow-hidden transition-all active:scale-[0.99] ${isStalled && showStalledOnly
 ?'border-amber-200 dark:border-amber-800 bg-amber-50/10'
 :'border-slate-100 dark:border-slate-700'
 }`}
 >
 {/* Left: Image */}
 <div className="w-16 h-16 rounded-xl bg-slate-100 dark:bg-slate-700 overflow-hidden flex-shrink-0 relative border border-slate-100 dark:border-slate-600">
 {product.imageUrl ? (
 ...
 <div className="flex items-center gap-1 bg-slate-50 dark:bg-slate-700 px-1.5 py-0.5 rounded text-[10px] font-mono border border-slate-100 dark:border-slate-600">
 ...
 <button onClick={() => handleEditClick(product)} className="text-slate-400 hover:text-primary dark:hover:text-primary"><ChevronRight size={18} /></button>
 <button onClick={() => handleDeleteProduct(product.id, product.name)} className="text-slate-300 hover:text-red-500 dark:text-slate-600 dark:hover:text-red-400"><Trash2 size={16} /></button>
 </div>
 </div>
 );
 })}
 {paginatedProducts.length === 0 && (
 <EmptyState icon={<Package size={48} />} title="Nenhum produto encontrado" description="Ajuste os filtros ou cadastre um novo produto." />
 )}
 </div>
 ...
 return (
 <tr key={product.id} className={`hover:bg-slate-50/50 dark:hover:bg-slate-700/30 transition-colors group ${isStalled && showStalledOnly ?'bg-amber-50/30 dark:bg-amber-900/10':''}`}>
 <td className="p-4 font-mono text-slate-500 dark:text-slate-400 text-xs">{product.sku}</td>
 <td className="p-4">
 ...
 <span className="text-[10px] text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-700 px-1.5 py-0.5 rounded">{product.category}</span>
 ...
 <div className="text-[10px] text-slate-400 dark:text-slate-500 flex flex-col">
 ...
 <td className="p-4">
 <div className="font-medium text-slate-900 dark:text-white">R$ {product.priceSale.toFixed(2)}</div>
 {!isSalesperson && (
 <>
 ...
 className="text-slate-400 hover:text-red-600 dark:hover:text-red-400 p-1 rounded-md hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors"
 title="Excluir"
 >
 <Trash2 size={18} />
 </button>
 ...
 {totalPages > 1 && (
 <div className="p-4 border-t border-slate-100 dark:border-slate-700 flex flex-col sm:flex-row justify-between items-center gap-4 bg-white dark:bg-slate-800">
 <span className="text-sm text-slate-500 dark:text-slate-400">
 Mostrando <span className="font-bold text-slate-800 dark:text-white">{(currentPage - 1) * itemsPerPage + 1}</span> a <span className="font-bold text-slate-800 dark:text-white">{Math.min(currentPage * itemsPerPage, filteredProducts.length)}</span> de <span className="font-bold text-slate-800 dark:text-white">{filteredProducts.length}</span> resultados
 </span>
 ...
 className="p-2 rounded-lg border border-slate-200 dark:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-700 disabled:opacity-50 disabled:cursor-not-allowed text-slate-600 dark:text-slate-300 transition-colors"
 >
 ...
 disabled={currentPage === 1}
 className="p-2 rounded-lg border border-slate-200 dark:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-700 disabled:opacity-50 disabled:cursor-not-allowed text-slate-600 dark:text-slate-300 transition-colors flex items-center gap-1"
 >
 <span className="hidden sm:inline">Anterior</span>
 <ChevronLeft size={18} />
 </button>

 <span className="px-4 py-2 bg-slate-50 dark:bg-slate-700 rounded-lg text-sm font-bold text-slate-800 dark:text-white border border-slate-200 dark:border-slate-600">
 {currentPage}
 </span>
 ...
 <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-sm"onClick={() => setIsAnalysisModalOpen(false)}></div>
 <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-4xl relative flex flex-col max-h-[90vh] overflow-hidden animate-in fade-in zoom-in-95 duration-200">
 ...
 <button onClick={() => setIsAnalysisModalOpen(false)} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200">
 <X size={24} />
 </button>
 </div>
 ...
 <thead className="bg-slate-50 dark:bg-slate-700/50 text-slate-500 dark:text-slate-400 uppercase text-xs sticky top-0">
 <tr>
 ...
 {product.name}
 <span className="block text-xs text-slate-400 font-normal">{product.sku}</span>
 </td>
 ...
 <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-3xl relative flex flex-col max-h-[90vh] overflow-hidden animate-in fade-in zoom-in-95 duration-200">
 {/* Header */}
 ...
 <button onClick={() => setIsModalOpen(false)} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-full p-1 transition-colors">
 <X size={20} />
 </button>
 </div>
 ...
 <div>
 <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Imagens do Produto (Até 8)</label>
 <div className="flex flex-wrap items-start gap-4">
 {/* Main Image */}
 <div className="flex flex-col gap-1">
 <span className="text-[10px] text-slate-500 font-medium ml-1">Principal</span>
 <div className="relative w-28 h-28 rounded-lg border-2 border-dashed border-slate-300 dark:border-slate-600 flex items-center justify-center bg-slate-50 dark:bg-slate-700 overflow-hidden group hover:border-primary/30 transition-colors shadow-sm">
 {newProduct.imageUrl ? (
 ...
 <span className="text-[10px] text-slate-500 font-medium ml-1">Extra {slotIndex}</span>
 <div className="relative w-20 h-20 mt-auto rounded-lg border-2 border-dashed border-slate-200 dark:border-slate-600 flex items-center justify-center bg-slate-50 dark:bg-slate-700/50 overflow-hidden group hover:border-primary/30 transition-colors">
 {imgUrl ? (
 <img loading="lazy" decoding="async"
 src={imgUrl}
 alt={`Extra ${slotIndex}`}
 ...
 <div className="flex-1 min-w-[120px] ml-2 text-xs text-slate-500 dark:text-slate-400 self-center">
 ...
 <p className="leading-tight text-[10px] text-slate-400">Clique nos slots para adicionar fotos secundárias.</p>
 </div>
 </div>
 </div>
 ...
 {/* Category Selection */}
 <div>
 <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Categoria do Produto</label>
 ...
 ?'border-primary/30 bg-primary/30/20 text-primary-700 dark:text-primary/70 ring-1 ring-[rgb(var(--primary) / 0.3)]'
 :'border-slate-200 dark:border-slate-600 dark:text-slate-300 hover:border-slate-300 dark:hover:border-slate-500'
 }`}
 >
 {cat}
 </div>
 ...
 <label className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Nome do Produto</label>
 <input
 required
 type="text"
 className="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-[rgb(var(--primary) / 0.3)]/50 focus:border-primary/30 outline-none transition-all bg-white dark:bg-slate-700 text-slate-900 dark:text-white"
 ...
 <div>
 <label className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Código (SKU)</label>
 ...
 <input
 type="text"
 className="w-full pl-8 pr-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-[rgb(var(--primary) / 0.3)]/50 focus:border-primary/30 outline-none transition-all bg-white dark:bg-slate-700 text-slate-900 dark:text-white"
 ...
 <div>
 <label className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Coleção</label>
 <input
 type="text"
 className="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-[rgb(var(--primary) / 0.3)]/50 focus:border-primary/30 outline-none transition-all bg-white dark:bg-slate-700 text-slate-900 dark:text-white"
 ...
 {/* Description */}
 <div>
 <label className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Descrição Detalhada</label>
 <textarea
 className="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-[rgb(var(--primary) / 0.3)]/50 focus:border-primary/30 outline-none transition-all h-20 resize-none text-sm bg-white dark:bg-slate-700 text-slate-900 dark:text-white"
 ...
 {/* PROMOTION CHECKBOX */}
 <div className={`p-4 rounded-lg border transition-colors ${newProduct.isPromotion ?'bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800':'bg-slate-50 dark:bg-slate-800/50 border-slate-200 dark:border-slate-700'}`}>
 ...
 />
 <span className={`font-medium flex items-center gap-2 ${newProduct.isPromotion ?'text-green-800 dark:text-green-300':'text-slate-600 dark:text-slate-400'}`}>
 ...
 <div>
 <label className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Preço Custo</label>
 <div className="relative">
 ...
 <input
 type="number"
 className="w-full pl-8 pr-3 py-2 border border-green-300 dark:border-green-600 rounded-lg focus:ring-2 focus:ring-green-400/50 focus:border-green-400 outline-none transition-all bg-green-50 dark:bg-green-900/20 text-slate-900 dark:text-white"
 placeholder="0.00"
 ...
 % Comissão <span className="text-slate-400">(Vend)</span>
 </label>
 <div className="relative">
 <input
 type="number"
 className="w-full pl-3 pr-8 py-2 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-[rgb(var(--primary) / 0.3)]/50 focus:border-primary/30 outline-none transition-all bg-white dark:bg-slate-700 text-slate-900 dark:text-white"
 placeholder="0"
 ...
 <div>
 <label className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Lucro Líquido</label>
 <div className="px-3 py-2 bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-lg text-sm text-emerald-600 dark:text-emerald-400 font-bold flex justify-between">
 <span>R$ {calcEstimatedProfit().toFixed(2)}</span>
 </div>
 </div>
 <div className="md:col-span-2">
 <label className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Qtd. Estoque {hasVariations &&'(Calculado Auto)'}</label>
 <input
 type="number"
 disabled={hasVariations}
 className={`w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg outline-none transition-all ${hasVariations ?'bg-slate-100 text-slate-400 cursor-not-allowed':'bg-white dark:bg-slate-700 text-slate-900 dark:text-white focus:ring-2 focus:ring-[rgb(var(--primary) / 0.3)]/50 focus:border-primary/30'}`}
 value={hasVariations ? variationsList.reduce((acc, v) => acc + (Number(v.stock) || 0), 0) : (newProduct.stock ||'')}
 onChange={e => setNewProduct({ ...newProduct, stock: parseInt(e.target.value) })}
 />
 </div>
 <div className="md:col-span-2">
 <label className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Estoque Mín.</label>
 <input
 type="number"
 className="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-[rgb(var(--primary) / 0.3)]/50 focus:border-primary/30 outline-none transition-all bg-white dark:bg-slate-700 text-slate-900 dark:text-white"
 ...
 <h4 className="font-bold text-slate-800 dark:text-white mb-4 flex items-center gap-2"><Plus size={16} /> Adicionar Variação</h4>
 <div className="grid grid-cols-2 md:grid-cols-6 gap-3 items-end">
 <div className="col-span-1">
 <label className="block text-xs font-medium text-slate-500 mb-1">Tipo</label>
 <CustomDropdown
  value={currentVariation.type}
  onChange={(v) => setCurrentVariation({ ...currentVariation, type: v })}
  options={variationTypes.map(t => ({ value: t, label: t }))}
  placeholder="Tipo"
/>
 </div>
 <div className="col-span-1">
 <label className="block text-xs font-medium text-slate-500 mb-1">Nome (ex: P, Azul)</label>
 <input
 type="text"
 className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg bg-white dark:bg-slate-700 dark:text-white dark:border-slate-600"
 ...
 className="w-full py-2 bg-primary text-white rounded-lg hover:bg-indigo-700 text-sm font-medium"
 >
 Adicionar
 </button>
 </div>
 ...
 <tr>
 <th className="p-3 font-medium text-slate-500 dark:text-slate-400">Tipo</th>
 <th className="p-3 font-medium text-slate-500 dark:text-slate-400">Nome</th>
 <th className="p-3 font-medium text-slate-500 dark:text-slate-400 text-center">Estoque</th>
 <th className="p-3 font-medium text-slate-500 dark:text-slate-400 text-center">Preço</th>
 <th className="p-3 font-medium text-slate-500 dark:text-slate-400 text-right">Ação</th>
 </tr>
 </thead>
 <tbody className="divide-y dark:divide-slate-700">
 {variationsList.map((v, idx) => (
 <tr key={idx} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 text-slate-800 dark:text-white">
 <td className="p-3">{v.type}</td>
 <td className="p-3 font-medium">{v.name}</td>
 <td className="p-3 text-center">{v.stock}</td>
 <td className="p-3 text-center">{v.priceOverride ? `R$ ${Number(v.priceOverride).toFixed(2)}` : <span className="text-slate-400 text-xs">Padrão</span>}</td>
 ...
 <tr>
 <td colSpan={5} className="p-8 text-center text-slate-400">Nenhuma variação adicionada ainda.</td>
 </tr>
 )}
 </tbody>
 </table>
 ...
 Total em Estoque: <span className="font-bold text-slate-800 dark:text-white">{variationsList.reduce((acc, v) => acc + (Number(v.stock) || 0), 0)}</span>
 </div>
 </div>
 )}
 ...
 <div className="absolute inset-0 bg-slate-900/50 backdrop-blur-sm"onClick={() => setIsCalcModalOpen(false)}></div>
 <div className="bg-white dark:bg-slate-800 w-full max-w-md rounded-xl shadow-2xl relative animate-in fade-in zoom-in-95 duration-200 border border-slate-200 dark:border-slate-700">
 ...
 <button onClick={() => setIsCalcModalOpen(false)} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200">
 <X size={20} />
 </button>
 </div>
 ...
 <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">Margem de Lucro Desejada (%)</label>
 <div className="relative">
 <input
 type="number"
 className="w-full pl-3 pr-8 py-2 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-primary/20 dark:focus:ring-indigo-900 outline-none bg-white dark:bg-slate-700 text-slate-900 dark:text-white"
 ...
 <div>
 <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">Taxa Cartão/Pix (%)</label>
 <input
 type="number"
 className="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg outline-none bg-white dark:bg-slate-700 text-slate-900 dark:text-white text-sm"
 value={calcData.taxCard}
 onChange={e => setCalcData({ ...calcData, taxCard: parseFloat(e.target.value) })}
 />
 </div>
 <div>
 <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">Taxa Plataforma (%)</label>
 <input
 type="number"
 className="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg outline-none bg-white dark:bg-slate-700 text-slate-900 dark:text-white text-sm"
 ...
 <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase text-center mb-2">Preço Sugerido de Venda</p>
 <div className="text-3xl font-bold text-indigo-600 dark:text-indigo-400 text-center bg-slate-50 dark:bg-slate-700/50 p-4 rounded-xl border border-indigo-100 dark:border-indigo-900/30">
 ...
 <div className="p-4 border-t border-slate-100 dark:border-slate-700 flex gap-3 bg-slate-50 dark:bg-slate-700/30">
 <button
 onClick={() => setIsCalcModalOpen(false)}
 className="flex-1 py-2 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-600 rounded-lg text-sm font-medium transition-colors"
 >
 Cancelar
 </button>
 <DSButton
 ...
 className="absolute -top-3 -right-3 bg-white dark:bg-slate-700 text-slate-600 dark:text-slate-200 rounded-full p-1.5 shadow-lg hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-900/30 dark:hover:text-red-400 transition-colors z-10"
 >
 <X size={20} />
 </button>
 <img loading="lazy" decoding="async" src={previewImage} alt="Preview"className="max-w-full max-h-[85vh] rounded-xl shadow-2xl object-contain"/>
 </div>
 </div>
 )}
 </div >
