...
const Bundles: React.FC = () => {
 ...
 toast.info("Adicione pelo menos 2 produtos ao kit.");
 return;
 }
 ...
 <p className="text-slate-500 dark:text-slate-400">Crie ofertas especiais agrupando produtos.</p>
 </div>
 <button
 onClick={() => {
 setEditingBundleId(null);
 ...
 return (
 <div key={bundle.id} className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden flex flex-col group hover:border-lime-300 transition-all relative cursor-pointer"onClick={() => handleEditBundle(bundle)}>
 {/* Action Buttons */}
 ...
 className="bg-white/80 dark:bg-black/50 p-1.5 rounded-lg text-slate-500 hover:text-red-500 hover:bg-white dark:hover:bg-slate-700 transition-all shadow-sm"
 title="Excluir Kit"
 >
 <Trash2 size={16} />
 </button>
 ...
 {bundle.imageUrl ? (
 <img loading="lazy" decoding="async" src={bundle.imageUrl} className="w-full h-full object-cover"/>
 ) : (
 <div className="w-full h-full flex items-center justify-center text-slate-400"><Layers size={32} /></div>
 )}
 ...
 <h3 className="font-bold text-lg text-slate-800 dark:text-white leading-tight">{bundle.name}</h3>
 <span className="text-xs font-mono text-slate-400 bg-slate-100 dark:bg-slate-700 px-1.5 py-0.5 rounded">{bundle.sku}</span>
 </div>

 <div className="space-y-2 mb-4 flex-1">
 <p className="text-xs font-bold text-slate-500 uppercase">Contém:</p>
 ...
 <div>
 <p className="text-xs text-slate-400 line-through">De: R$ {realValue.toFixed(2)}</p>
 ...
 <div className="bg-white dark:bg-slate-800 w-full max-w-5xl h-[90vh] rounded-xl shadow-2xl relative flex flex-col md:flex-row overflow-hidden animate-in fade-in zoom-in-95 duration-200">

 {/* LEFT: FORM & PREVIEW (Step 2 on Mobile) */}
 <div className={`w-full md:w-1/2 p-6 flex flex-col border-r border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 overflow-y-auto shrink-0 md:shrink ${isMobile && wizardStep !== 2 ?'hidden':''}`}>
 <div className="flex justify-between items-center mb-6">
 <div className="flex items-center gap-2">
 <button
 onClick={() => setWizardStep(1)}
 className="md:hidden p-1 rounded-full hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-500"
 >
 <ArrowLeft size={20} />
 </button>
 ...
 <button onClick={() => setIsModalOpen(false)} className="md:hidden text-slate-400">
 <X size={24} />
 </button>
 </div>
 ...
 <div>
 <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">Nome do Kit</label>
 <input
 required
 type="text"
 className="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-lime-200 focus:border-lime-500 outline-none bg-white dark:bg-slate-800 text-slate-900 dark:text-white"
 ...
 <div>
 <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">SKU (Cód)</label>
 <input
 type="text"
 className="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-lime-200 focus:border-lime-500 outline-none bg-white dark:bg-slate-800 text-slate-900 dark:text-white"
 placeholder="KIT-001"
 ...
 <div>
 <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">Preço Venda (Promo)</label>
 <input
 required
 type="number"
 className="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-lime-200 focus:border-lime-500 outline-none bg-white dark:bg-slate-800 text-slate-900 dark:text-white font-bold text-lime-700"
 placeholder="0.00"
 ...
 <h4 className="text-sm font-bold text-slate-800 dark:text-white mb-3">Itens do Kit ({bundleForm.bundleComponents?.length})</h4>
 ...
 return (
 <div key={comp.productId} className="flex items-center justify-between bg-slate-50 dark:bg-slate-700/50 p-2 rounded-lg text-sm">
 <div className="flex-1 truncate pr-2 dark:text-slate-300">
 {product.name}
 </div>
 <div className="flex items-center gap-2">
 <input
 type="number"
 min="1"
 className="w-12 px-1 py-0.5 border rounded text-center text-xs bg-white dark:bg-slate-600 dark:border-slate-500 dark:text-white"
 value={comp.quantity}
 ...
 <span className="font-medium dark:text-slate-300">R$ {calculateBundleCost(bundleForm.bundleComponents).toFixed(2)}</span>
 </div>
 <div className="flex justify-between text-xs mb-1">
 <span className="text-slate-500 dark:text-slate-400">Preço Real (Soma):</span>
 <span className="font-medium line-through dark:text-slate-300">R$ {calculateBundleRealValue(bundleForm.bundleComponents).toFixed(2)}</span>
 </div>
...
'--'
 ...
 <div className={`w-full md:w-1/2 p-6 flex flex-col bg-white dark:bg-slate-800 flex-1 min-h-0 ${isMobile && wizardStep !== 1 ?'hidden':''}`}>
 <div className="flex justify-between items-center mb-4">
 <h4 className="font-bold text-slate-700 dark:text-slate-300">Adicionar Produtos</h4>
 ...
 className="w-full pl-10 pr-4 py-2 border border-slate-200 dark:border-slate-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-lime-200 bg-slate-50 dark:bg-slate-700 text-slate-900 dark:text-white"
 value={searchTerm}
 ...
 :'bg-white border-slate-100 hover:border-lime-300 dark:bg-slate-700 dark:border-slate-600 dark:hover:border-lime-500'
 }`}
 >
 <div className="flex items-center gap-3 overflow-hidden">
 <div className="w-10 h-10 bg-slate-100 dark:bg-slate-600 rounded flex-shrink-0">
 {product.imageUrl && <img loading="lazy" decoding="async" src={product.imageUrl} className="w-full h-full object-cover rounded"/>}
 </div>
 ...
 <div className={`w-6 h-6 rounded-full flex items-center justify-center ${isAdded ?'bg-lime-500 text-white':'bg-slate-100 text-slate-400 dark:bg-slate-600'}`}>
 ...
 className="text-xs text-slate-500 disabled:opacity-30 hover:text-lime-600"
 >
 Anterior
 </button>
 ...
 <div className="hidden md:block mt-4 pt-4 border-t border-slate-100 dark:border-slate-700">
 <button
 form="bundle-form"
 type="submit"
