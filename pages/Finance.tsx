...
const Finance: React.FC<{ user?: any; transactions?: any[] }> = ({ user }) => {
 ...
 if (!amountVal || amountVal <= 0) { toast.info('Valor inválido.'); return; }
 if (!sangriaForm.description.trim()) { toast.info('Informe uma descrição.'); return; }
 ...
 <div>
 <h2 className="text-2xl font-bold text-slate-800 dark:text-white">Financeiro</h2>
 <p className="text-slate-500 dark:text-slate-400">Fluxo de caixa, detalhamento de entradas e saídas.</p>
 </div>
 <button
 ...
 <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm flex flex-col justify-between transition-colors">
 <div className="flex justify-between items-start">
 <div>
 <p className="text-slate-500 dark:text-slate-400 font-bold mb-1 uppercase text-xs tracking-wider">Saldo Líquido</p>
 <h3 className={`text-3xl font-black ${balance >= 0 ?'text-slate-800 dark:text-white':'text-red-600 dark:text-red-400'}`}>R$ {balance.toFixed(2)}</h3>
 </div>
 <div className="p-2 bg-indigo-50 dark:bg-indigo-900/30 rounded-lg text-indigo-600 dark:text-indigo-400">
 <Wallet size={24} />
 </div>
 </div>
 <p className="text-xs text-slate-400 mt-4 font-medium">Resultado da operação</p>
 </div>
 </div>

 {/* VENDAS POR CANAL - LOJA / WHATSAPP / ONLINE */}
 <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
 <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-primary/30/50 shadow-sm flex items-center gap-4 relative overflow-hidden">
 <div className="absolute right-0 top-0 h-full w-1.5 bg-primary/30"/>
 <div className="p-3 bg-primary/30/20 rounded-full">
 <Store size={22} className="text-[#e0888b]"/>
 </div>
 <div className="flex-1">
 <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase">Loja Física</p>
 <p className="text-2xl font-black text-slate-800 dark:text-white">R$ {storeTotalFull.toFixed(2)}</p>
 <p className="text-xs text-slate-400 mt-0.5">{records.filter(r => r.type ==='Income'&& (!r.source || r.source ==='store')).length} pedidos</p>
 </div>
 </div>
 <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-emerald-100 dark:border-emerald-900/30 shadow-sm flex items-center gap-4 relative overflow-hidden">
 ...
 <div className="flex-1">
 <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase">WhatsApp / Instagram</p>
 <p className="text-2xl font-black text-slate-800 dark:text-white">R$ {whatsappTotal.toFixed(2)}</p>
 <p className="text-xs text-slate-400 mt-0.5">{records.filter(r => r.type ==='Income'&& r.source ==='whatsapp').length} pedidos</p>
 </div>
 </div>
 <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-indigo-100 dark:border-indigo-900/30 shadow-sm flex items-center gap-4 relative overflow-hidden">
 ...
 <div className="flex-1">
 <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase">Online (Site)</p>
 <p className="text-2xl font-black text-slate-800 dark:text-white">R$ {onlineTotal.toFixed(2)}</p>
 <p className="text-xs text-slate-400 mt-0.5">{records.filter(r => r.type ==='Income'&& r.source ==='online').length} pedidos</p>
 </div>
 </div>
 </div>
 ...
 <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-teal-100 dark:border-teal-900/30 shadow-sm flex items-center gap-4 relative overflow-hidden">
 ...
 <div>
 <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase">PIX</p>
 <p className="text-xl font-bold text-teal-700 dark:text-teal-400">R$ {incomePix.toFixed(2)}</p>
 </div>
 </div>
 <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-indigo-100 dark:border-indigo-900/30 shadow-sm flex items-center gap-4 relative overflow-hidden">
 ...
 <div>
 <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase">Crédito</p>
 <p className="text-xl font-bold text-indigo-700 dark:text-indigo-400">R$ {incomeCredit.toFixed(2)}</p>
 </div>
 </div>
 <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-blue-100 dark:border-blue-900/30 shadow-sm flex items-center gap-4 relative overflow-hidden">
 ...
 <div>
 <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase">Débito</p>
 <p className="text-xl font-bold text-blue-700 dark:text-blue-400">R$ {incomeDebit.toFixed(2)}</p>
 </div>
 </div>
 <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-emerald-100 dark:border-emerald-900/30 shadow-sm flex items-center gap-4 relative overflow-hidden">
 ...
 <div>
 <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase">Dinheiro</p>
 ...
 {/* Header + Filters */}
 <div className="p-4 border-b border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-700/30 space-y-3">
 <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
 <h3 className="font-bold text-slate-800 dark:text-white">Lançamentos Recentes</h3>
 <span className="text-xs text-slate-400">{filteredRecords.length} registro(s)</span>
 </div>
 ...
 className="w-full pl-8 pr-3 py-2 text-sm border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-800 dark:text-white focus:outline-none focus:ring-1 focus:ring-[rgb(var(--primary) / 0.3)]"
 />
 </div>
 {/* Payment filter */}
 <CustomDropdown
  className="flex-1"
  value={filterPayment}
  onChange={(v) => handleFilterChange(setFilterPayment, v)}
  options={[
...
/>
 ...
 className="px-3 py-2 text-xs bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-600 flex items-center gap-1 whitespace-nowrap"
 >
 <X size={12} /> Limpar
 </button>
 )}
 ...
 <td className="p-4 font-medium text-slate-800 dark:text-white max-w-[200px]">
 <span className="line-clamp-1">{item.description}</span>
 </td>
 <td className="p-4">
 {item.sellerName && item.sellerName !=='--'? (
 <span className="flex items-center gap-1 text-slate-700 dark:text-slate-300 text-xs font-medium">
 ...
 className="px-2 py-1 text-xs bg-primary/30/20 hover:bg-primary/30/40 text-[#c86065] dark:text-primary/70 border border-primary/30/40 rounded-lg flex items-center gap-1 font-bold transition-colors"
 >
 ...
 className="p-1.5 rounded-lg border border-slate-200 dark:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-700 disabled:opacity-40 text-slate-600 dark:text-slate-300"
 >
 <ChevronLeft size={16} />
 </button>
 ...
 className={`w-8 h-8 text-xs rounded-lg border font-bold transition-colors ${currentPage === page ?'bg-primary/30 border-primary/30 text-slate-900':'border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700'}`}
 >
 {page}
 </button>
 );
 ...
 <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-sm overflow-hidden animate-in fade-in zoom-in-95">
 <div className="p-4 bg-primary/30 flex justify-between items-center">
 <h3 className="font-bold text-slate-900 flex items-center gap-2"><Receipt size={18} /> Comprovante da Venda</h3>
 <button onClick={() => setReceiptSale(null)} className="text-slate-700 hover:text-slate-900"><X size={20} /></button>
 </div>
 <div className="p-5 space-y-3 text-sm">
 <div className="flex justify-between">
 <span className="text-slate-500">Cliente</span>
 <span className="font-bold text-slate-800 dark:text-white">{receiptSale.customerName}</span>
 </div>
 {(receiptSale as any).sellerName && (
 <div className="flex justify-between">
 <span className="text-slate-500">Vendedora</span>
 <span className="font-bold text-slate-800 dark:text-white">{(receiptSale as any).sellerName}</span>
 </div>
 )}
 <div className="flex justify-between">
 <span className="text-slate-500">Data</span>
 <span className="font-medium text-slate-700 dark:text-slate-300">{new Date(receiptSale.date).toLocaleDateString('pt-BR', { day:'2-digit', month:'2-digit', year:'numeric', hour:'2-digit', minute:'2-digit'})}</span>
 </div>
 <div className="flex justify-between">
 ...
 <div className="border-t border-slate-100 dark:border-slate-700 pt-3 mt-2 space-y-1">
 ...
 <span className="font-medium text-slate-700 dark:text-slate-300">R$ {(item.priceSale * item.quantity).toFixed(2)}</span>
 </div>
 ))}
 </div>
 )}
 ...
 <div className="flex justify-between text-base font-black text-slate-900 dark:text-white border-t border-slate-200 dark:border-slate-600 pt-3">
 <span>TOTAL</span>
 ...
 <DSButton onClick={() => setReceiptSale(null)} variant="primary">Fechar</DSButton>
 </div>
 </div>
 </div>
 )}
 ...
 <div className="bg-white dark:bg-slate-800 w-full max-w-sm rounded-xl shadow-2xl relative animate-in fade-in zoom-in-95 duration-200">
 <div className="p-5 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center bg-rose-50 dark:bg-rose-900/20">
 ...
 <p className="text-center text-sm text-slate-600 dark:text-slate-400 mb-4">Esta ação requer autorização.</p>
 <div>
 <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">Senha</label>
 <input type="password"className="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-rose-500 outline-none bg-white dark:bg-slate-700 text-slate-900 dark:text-white"value={adminPassword} onChange={e => setAdminPassword(e.target.value)} placeholder="Digite a senha..."autoFocus />
 </div>
 ...
 {isSangriaModalOpen && (
 <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
 <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-sm"onClick={() => setIsSangriaModalOpen(false)} />
 <div className="bg-white dark:bg-slate-800 w-full max-w-md rounded-xl shadow-2xl relative animate-in fade-in zoom-in-95 duration-200">
 ...
 <h3 className="font-bold text-lg text-slate-800 dark:text-white flex items-center gap-2"><MinusCircle size={20} className="text-rose-500"/> Registrar Sangria</h3>
 <button onClick={() => setIsSangriaModalOpen(false)} className="text-slate-400 hover:text-slate-600"><X size={20} /></button>
 </div>
 ...
 <div>
 <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">Forma de Retirada</label>
 <div className="grid grid-cols-2 gap-3">
 <div onClick={() => setSangriaForm({ ...sangriaForm, method:'Dinheiro'})} className={`cursor-pointer border rounded-lg p-3 flex flex-col items-center gap-1 transition-all ${sangriaForm.method ==='Dinheiro'?'bg-rose-100 border-rose-500 text-rose-700 dark:bg-rose-900/40 dark:text-rose-300':'bg-white dark:bg-slate-700 border-slate-300 dark:border-slate-600'}`}>
 <Banknote size={20} /><span className="text-xs font-bold">Dinheiro</span>
 </div>
 <div onClick={() => setSangriaForm({ ...sangriaForm, method:'Pix'})} className={`cursor-pointer border rounded-lg p-3 flex flex-col items-center gap-1 transition-all ${sangriaForm.method ==='Pix'?'bg-teal-100 border-teal-500 text-teal-700 dark:bg-teal-900/40 dark:text-teal-300':'bg-white dark:bg-slate-700 border-slate-300 dark:border-slate-600'}`}>
 <QrCode size={20} /><span className="text-xs font-bold">Pix</span>
 </div>
 </div>
 </div>
 <div>
 <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">Valor da Retirada (R$)</label>
 <input type="number"step="0.01"className="w-full px-3 py-3 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-rose-500 outline-none bg-white dark:bg-slate-700 text-slate-900 dark:text-white text-lg font-bold"value={sangriaForm.amount} onChange={e => setSangriaForm({ ...sangriaForm, amount: e.target.value })} placeholder="0.00"autoFocus />
 </div>
 <div>
 <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">Motivo / Descrição</label>
 <input type="text"className="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-rose-500 outline-none bg-white dark:bg-slate-700 text-slate-900 dark:text-white"value={sangriaForm.description} onChange={e => setSangriaForm({ ...sangriaForm, description: e.target.value })} placeholder="Ex: Depósito Bancário, Pagamento Fornecedor..."/>
 </div>
