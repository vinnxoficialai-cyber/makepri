...
type ValidationType ='email'|'cpf'|'phone'|'required'|'none';

// Inline mask functions
const masks = {
 ...
 phone: (v: string) => { const d = v.replace(/\D/g,''); return d.length <= 10 ? d.replace(/(\d{2})(\d)/,'($1) $2').replace(/(\d{4})(\d)/,'$1-$2') : d.replace(/(\d{2})(\d)/,'($1) $2').replace(/(\d{5})(\d)/,'$1-$2'); },
 cep: (v: string) => v.replace(/\D/g,'').replace(/(\d{5})(\d)/,'$1-$2').slice(0, 9),
 currency: (v: string) => { const n = v.replace(/\D/g,''); const f = (parseInt(n ||'0') / 100).toFixed(2); return `R$ ${f.replace('.',',').replace(/\B(?=(\d{3})+(?!\d))/g,'.')}`; },
 date: (v: string) => v.replace(/\D/g,'').replace(/(\d{2})(\d)/,'$1/$2').replace(/(\d{2})(\d)/,'$1/$2').slice(0, 10),
};

// Inline validators
const validators = {
 email: (v: string) => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(v),
 cpf: (v: string) => v.replace(/\D/g,'').length === 11,
 phone: (v: string) => { const d = v.replace(/\D/g,''); return d.length >= 10 && d.length <= 11; },
};
...
export const FormInput: React.FC<FormInputProps> = ({
 label, error: externalError, hint, mask ='none', validation ='none',
 showValidation = true, isDarkMode = false, onChange, value, className, ...props
}) => {
 ...
 const fn = validators[validation as keyof typeof validators];
 ...
 onBlur={handleBlur}
 className={`w-full ${bgInput} border ${borderClass} rounded-lg p-2.5 pr-10 text-sm ${textMain}
 ...
 <AlertCircle size={12} />
 {externalError || getValidationMessage(validation as Exclude<ValidationType,'none'>)}
 </p>
 )}
 {hint && !hasError && (
...
function getValidationMessage(validation: Exclude<ValidationType,'none'>): string {
 switch (validation) {
 case'email': return'E-mail invalido';
 case'cpf': return'CPF invalido';
 case'phone': return'Telefone invalido';
 case'required': return'Campo obrigatorio';
...
export const FormTextarea: React.FC<FormTextareaProps> = ({
 label, error, hint, isDarkMode = false, onChange, value, className, ...props
}) => {
 const bgInput = isDarkMode ?'bg-dark':'bg-white';
 const textMain = isDarkMode ?'text-white':'text-slate-800';
 const textSub = isDarkMode ?'text-slate-400':'text-slate-500';
 ...
 className={`w-full ${bgInput} border ${borderClass} rounded-lg p-2.5 text-sm ${textMain}
 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary
...
export const FormSelect: React.FC<FormSelectProps> = ({
 label, options, error, hint, isDarkMode = false, onChange, value, className, ...props
}) => {
 const textSub = isDarkMode ?'text-slate-400':'text-slate-500';
