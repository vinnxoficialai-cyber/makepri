...
export interface DropdownOption { value: string; label: string; icon?: React.ReactNode; dot?: string; }

interface Props {
    value: string; onChange: (v: string) => void; options: DropdownOption[];
    placeholder?: string; icon?: React.ReactNode; isDarkMode?: boolean; className?: string; disabled?: boolean;
}

export const CustomDropdown: React.FC<Props> = ({ value, onChange, options, placeholder, icon, isDarkMode: prop, className = '', disabled = false }) => {
    const isDarkMode = prop ?? document.documentElement.classList.contains('dark');
    ...
    const handleToggle = () => {
        ...
        if (!open && triggerRef.current) {
            ...
            const menuH = Math.min(options.length * 36 + 12, 240);
            const above = spaceBelow < menuH + 8 && rect.top > menuH + 8;
            setMenuPos({ top: above ? rect.top - menuH - 6 : rect.bottom + 6, left: rect.left, width: rect.width });
        }
        setOpen(p => !p);
    };
    ...
    return (
        <div className={`relative ${className}`}>
          ...
          ${open
                ...
                <span className={`flex-1 truncate ${isDarkMode ? 'text-slate-200' : 'text-slate-700'}`}>{selected?.label || placeholder || 'Selecionar'}</span>
